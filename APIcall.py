...
def apicallcharID(charID):
	...
	for item in jsontext['data']['results']:  # gaat zoeken in data, dan results
		gekozenSuperheld = item[
			'name']  # print name uit data\results, bekijk https://developer.marvel.com/docs#!/public/getCreatorCollection_get_0 voor meer info
		superheldDescription = item['description']
		superheld = item['name']
		str(superheld).lower()
		print(superheld)
		print(superheldDescription)
